...
async function loadmovies(searchterm){
    const URL = `https://www.omdbapi.com/?s=${searchterm}&page=1&apikey=97c55a11`;
...
function loadmoviesdetails(){
    ...
    searchmovie.forEach(movies =>{
        movies.addEventListener('click',async () =>{
            search.classList.add('hide-search-list');
            moviesearch.value = "";
            const reasult = await fetch(`https://www.omdbapi.com/?i=${movies.dataset.id}&apikey=97c55a11`);
...
function makeslideshow(slidesID){
    ...
    const slidesInner = slides.getElementsByClassName('slides-inner');
...
let url = 'https://api.themoviedb.org/3/movie/popular?api_key=97c55a11d5b78932675dd9b9fd247a&language=en-US&page=1&region=IN';
...
function showmovie(movies){
    ...
    movies.forEach((movie)=>{
        ...
        <h4 id="h41">Rating : ${movie.vote_average}</h4>
        `
